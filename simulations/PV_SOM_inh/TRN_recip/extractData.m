...
if x == 1 || x==37 || x == 73
    ...
    for counter = ii
        tspks = MGBspks{1,counter};
        spks1 = [spks1, [tspks';repmat(counter,1,length(tspks))]];
        tspks = HOspks{1,counter};
        spks2 = [spks2, [tspks';repmat(counter,1,length(tspks))]];
        tspks = PVspks{1,counter};
        spks3 = [spks3, [tspks';repmat(counter,1,length(tspks))]];
        tspks = SOMspks{1,counter};
        spks4 = [spks4, [tspks';repmat(counter,1,length(tspks))]];
    end
end
...
%
subplot(2,2,1);
plot(spks1(1,:),spks1(2,:)-0.5,'k.','MarkerSize',8);
subplot(2,2,2);
plot(spks2(1,:),spks2(2,:)-0.5,'k.','MarkerSize',8);
subplot(2,2,3);
plot(spks3(1,:),spks3(2,:)-0.5,'k.','MarkerSize',8);
subplot(2,2,4);
plot(spks4(1,:),spks4(2,:)-0.5,'k.','MarkerSize',8);
%}
...
h.Title="MGB {\Delta}FR (PV Silenced)";
h.XData=0:0.2:1;
h.XLabel="SST lateral ratio";
h.YData=1:-0.2:0;
h.YLabel="PV lateral ratio";
h.Colormap=cmap;
h.ColorLimits=[-0.5,0.5];


figure(2);h2=heatmap(flipud(dMGB_FR_SOM));
h2.Title="MGB {\Delta}FR (SST Silenced)";
h2.XData=0:0.2:1;
h2.XLabel="SST lateral ratio";
h2.YData=1:-0.2:0;
h2.YLabel="PV lateral ratio";
h2.Colormap=cmap;
