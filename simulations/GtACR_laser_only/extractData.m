...
for i=1:n_trials
    MGBspks{1,i} = Sim_results(i+(n_trials*(x-1))).analysis.spktime.TC_MGB; %#ok<*SAGROW>
    HOspks{1,i} = Sim_results(i+(n_trials*(x-1))).analysis.spktime.TC_HO;
    PVspks{1,i} = Sim_results(i+(n_trials*(x-1))).analysis.spktime.TRN_PV;
    SOMspks{1,i} = Sim_results(i+(n_trials*(x-1))).analysis.spktime.TRN_SOM;
    totalSpks1 = [totalSpks1 MGBspks{1,i}']; %#ok<*AGROW>
    %totalSpks2 = [totalSpks2 HOspks{1,i}'];
    %totalSpks3 = [totalSpks3 PVspks{1,i}'];
...
for counter = 1:10
spks1{ii(counter),1} = MGBspks{1,counter}';
spks2{ii(counter),1} = HOspks{1,counter}';
spks3{ii(counter),1} = PVspks{1,counter}';
spks4{ii(counter),1} = SOMspks{1,counter}';
end
%}

[psth_MGB, centers] = return_histogram(totalSpks1, 1000, n_trials, 31);
